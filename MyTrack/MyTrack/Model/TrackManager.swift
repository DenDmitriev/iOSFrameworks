...
class TrackManager {
    ...
    var start: GMSMarker? {
        willSet {
            ...
            newValue?.snippet = description(location: newValue?.position, date: startTime ?? Date.now)
        }
    }
    var finish: GMSMarker? {
        willSet {
            newValue?.title = "Finish"
            newValue?.snippet = description(location: newValue?.position, date: finishTime ?? Date.now)
        }
    }
    ...
    init(start: GMSMarker? = nil, finish: GMSMarker? = nil, startTime: Date? = nil, finishTime: Date? = nil, locations: [CLLocation] = [], distance: Double = 0) {
